...
- `activeReleaseId`
- domain bindings
- preview URLs (optional runtime bindings to pinned releases)
...
### 2.9 Release preview (optional runtime capability)
...
- Preview configuration is server-side only:
  - `preview.enabled`
  - `preview.baseDomain`
...
- Stored fields:
  ...
  - `expires_at`
  - `created_at`
- Hostnames are generated by htmlservd, not supplied by clients:
  - `<token>--<env>--<website>.<preview.baseDomain>`
- Preview semantics:
  - preview host roots at `websites/<website>/envs/<env>/releases/<releaseId>`
  - preview hosts do not follow later `current` symlink changes
  - preview hosts emit `X-Robots-Tag: noindex, nofollow, noarchive`
  - preview hosts reuse the environment's backend and telemetry routing
...
- telemetry host attribution trust model:
  - ingest host is accepted only when it matches an existing domain binding or an active preview hostname.
...
- Preview binding per release:
  - `abc123--staging--sample.preview.example.com` -> staging release `R1` directory
...
- When previews are enabled, generated preview site blocks also include:
  - release-pinned roots under `releases/<id>` instead of `current`
  - `header X-Robots-Tag "noindex, nofollow, noarchive"`
  - the same telemetry and backend directives as the source environment
