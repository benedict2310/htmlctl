...
func TestValidateUpstreamURL(t *testing.T) {
	tests := []struct {
		...
		{name: "trailing slash", input: " https://auth.internal.example.com/ ", want: "https://auth.internal.example.com"},
		{name: "empty", input: "", wantErr: "required"},
		{name: "relative", input: "api.example.com", wantErr: "absolute"},
		{name: "ftp", input: "ftp://api.example.com", wantErr: "http or https"},
		{name: "path base", input: "https://auth.internal.example.com/base", wantErr: "must not include a path"},
