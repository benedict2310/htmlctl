...
func TestParseListReleasesPagination(t *testing.T) {
	req := httptest.NewRequest("GET", "/api/v1/websites/futurelab/environments/staging/releases", nil)
	...
	req = httptest.NewRequest("GET", "/api/v1/websites/futurelab/environments/staging/releases?limit=999&offset=3", nil)
	limit, offset, err = parseListReleasesPagination(req)
	if err != nil {
	...
	req = httptest.NewRequest("GET", "/api/v1/websites/futurelab/environments/staging/releases?limit=-1", nil)
	if _, _, err := parseListReleasesPagination(req); err == nil {
		t.Fatalf("expected negative limit error")
	}
	req = httptest.NewRequest("GET", "/api/v1/websites/futurelab/environments/staging/releases?offset=-1", nil)
	if _, _, err := parseListReleasesPagination(req); err == nil {
